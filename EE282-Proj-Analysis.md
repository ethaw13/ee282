...
The datasets will be acquired from online databases like UniProt and Blast and can be downloaded with the functions `curl` or `wget`. Additionally, HIV-1 curated sequences can be found in the Los Alamos HIV sequence database. Together, these resources provide a large amount of protein sequences that enable comparisons in sequences spanning geographic regions, yearly genetic drift, and small variations in the capsid protein sequence. The sequences gained from these online resources will first have to be truncated and aligned to fit the residues of interest. More specifically, the residues 1-221 of the CA protein will be analyzed. This standardized alignment will allow residue-residue comparisons of conservation and variability.

## Analyses

Multiple sequence alignment will be generated to enable residue comparisons across HIV-1 capsid protein sequences. Utilization of the linux package MAFFT (Katoh et al. 2013) alignments executed from bash, enabling uniform comparisons across HIV-1 capsid protein sequences. The resulting aligned FASTA file can be imported into R for data manipulation. Sequence positional conservation scores (Valdar et al. 2002) will be derived from the amino acid positional frequencies and Shannon entropy across each position. High conservation will indicate essential residues for function and must persist throughout mutation to main functionality. Low conservation scores will reveal amino acids susceptible to mutation. Sequence logos (Schneider et al. 1990) may also be made to visualize preferred amino acids at a certain position. These conservation values will be mapped onto three dimensional structures using visualization tools like VMD, enabling evolutionary patterns to be interpreted in the context of capsid assembly and protein interactions.

## Visualization

To visualize the analyses, we will employ bash scripting for the preprocessing alignment, and RStudio to do the conservation calculations. We will be using the ggplot library in R to plot the conservation score as a function of residue positions. Additionally, mapping these metrics to a three-dimensional structure is important to answer the overarching question. Although not the methods taught in class, tcl scripting serves as a powerful tool to manipulate coordinate informations, like overlaying protein structures and calculating root mean squared differences (RMSD). To visualize certain structures and their differences due to sequence variation, VMD (Humphrey et al. 1996) has the ability to render capsid variants to provide a clearer understanding of protein structure. To visualize variant structures, we can use AlphaFold3 (Abramson et al. 2024) to provide structural context for large sequence databases, adding some structural significance to interpreting evolutionary concepts.
